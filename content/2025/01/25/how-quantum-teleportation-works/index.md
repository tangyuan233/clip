...
> **摘要**:
>  这篇文章探讨了量子传送(quantum teleportation)的原理及其在量子计算中的重要性。文章通过一个墨西哥民间故事引入引人入胜的传送概念，接着解释了1993年物理学家提出的量子传送协议。虽然初始理论基于量子力学的数学规则，后续实验确认了这一现象。文章详细描述了传送协议的步骤，强调了 Alice 如何通过经典信息将她的量子态传送给 Bob。量子传送虽然表面上看似复杂，实际操作上却较为直接，涉及量子门操作和测量。文中强调了传送并非复制量子态，Alice 在完成传送后不再拥有源态。本文采用了一种独特的记忆媒体形式帮助读者长期记忆相关内容，并提出量子传送的奇特特性。
> 
>  **要点总结**:
>  1. 量子传送是一种通过量子纠缠和经典通信将量子态从一个位置传输到另一个位置的过程。
>  2. 传送协议的基本步骤包括 Alice 通过量子门和测量产生经典信息，然后传递给 Bob。
>  3. 传送过程中使用的量子态需要通过共享的纠缠态来实现。
>  4. 量子传送并不允许超光速通信，因为传递量子态仍需经典信息的传送。
>  5. 量子传送在量子计算和量子信息科学中具有重要应用，是理论与实验研究的核心内容。
