...
> **摘要**:
>  本文讲述了量子传送的故事和机制，首先通过一个墨西哥民间故事引入量子传送的概念，接着介绍了1993年物理学家们发现的量子传送现象，这种现象能在不直接发送量子态的情况下，将量子态传送到远方。文章详细阐述了量子传送的协议，涉及量子比特的状态、测量和经典信息的传递，强调该协议的简洁性和神奇性。通过Alice和Bob的协作，Alice可以将她的未知量子态传送到Bob那，并且Bob借助简单的经典信息来重建这个量子态。此外，文章还探讨了量子传送的潜在应用，包括减少量子计算中的噪声。最后，作者提到量子传送已经在多个实验中得以实现，并展望了它在未来量子计算中的重要性。
> 
>  **要点总结**:
>  1. 量子传送的起源是1993年物理学家的理论发现，可以在不传送量子态的情况下实现量子态的远程传递。
>  2. 量子传送协议的工作机制涉及Alice和Bob之间的量子纠缠、量子门的操作与经典信息传递。
>  3. 通过特定测量和操作，Alice能够将量子态以经典信息的形式传递给Bob，后者可以重建原量子态。
>  4. 量子传送对量子计算有深远影响，尤其是在减少噪声和量子信息处理方面。
>  5. 实验上已经验证了量子传送的可行性，并探讨了其在量子计算中的应用潜力。
