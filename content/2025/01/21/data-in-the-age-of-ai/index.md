...
> **摘要**:  
>  本文探讨了人工智能（AI）的爆发如何影响数据及数据业务。作者提出了两个主要观点：一是信息和计算的相对价值发生了重大变化，影响了软件和数据业务模式；二是数据和计算能力的绝对数量显著增加，影响了信任、身份、质量和数据管理。文章回顾了过去十年数据爆炸的历程，指出数据循环是推动最佳商业模式和AI革命的关键。随着硬件成本下降和存储技术的进步，数据量急剧增加，推动了广告、电子商务、物流等行业的快速发展。GPT等生成式AI的出现进一步提升了数据的价值，并催生了新的数据生态系统。未来，数据的质量和信任链将成为关键，而计算能力的爆炸式增长也将带来新的挑战和机遇。
> 
>  **要点总结**:
>  1. 信息和计算的相对价值变化对软件和数据业务模式产生了深远影响。
>  2. 数据和计算能力的绝对数量显著增加，影响了信任、身份、质量和数据管理。
>  3. 过去十年的数据爆炸推动了广告、电子商务、物流等行业的快速发展。
>  4. GPT等生成式AI的出现提升了数据的价值，并催生了新的数据生态系统。
>  5. 未来，数据的质量、信任链和计算能力的爆炸式增长将成为关键挑战和机遇。
