...
> **摘要**: 文章探讨了人工智能(AI)的突然崛起如何改变数据及数据业务，主要提出两个变化：一是信息和计算之间相对价值的重大变化，二是可用数据和计算的绝对数量的显著增加。作者回顾了数据爆炸带来的商业模式创新，如广告和物流等生态系统的形成，强调数据与软件的完全互补性质。随着GPT等技术的出现，数据的价值大幅提升，并将鼓励公司更有效地货币化独特和潜在的数据资产。此外，文章指出在这个AI时代，信息的丰富性将导致对创建和维护信任链、数据质量以及新的商业生态系统的重视，最终预示着计算革命的来临以及相应的影响。
> 
>  **要点总结**:
>  1. AI的崛起导致信息和计算之间的相对价值发生重大变化，推高数据价值。
>  2. 数据的绝对数量激增，促进了新的商业模式和高价值数据资产的形成。
>  3. 数据和软件互为补充，双方的价格变化将影响整个行业经济结构。
>  4. 随着数据的丰富，信任链的形成及其重要性将愈加凸显。
>  5. 计算革命将改变产业格局，推动生成模型的普及和数据消费方式的转变。
