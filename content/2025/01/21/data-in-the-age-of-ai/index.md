...
> **摘要**: 本文探讨了人工智能（AI）爆炸性增长对数据及数据业务的影响，提出了两个主要观点：一是信息和计算的相对价值发生了重大变化，进而影响了软件和数据商业模式；二是世界上数据和计算的绝对数量显著增加，这影响了信任、身份、质量和策展等问题。文章回顾了数据在过去十年如何推动商业模式，包括社交媒体、电子商务和其基础设施。数据和软件是相互依存的，生产力飞轮促进了两者的增强。GPT等生成模型极大提升了软件开发效率，使每位程序员的潜在生产力倍增。数据的价值因计算能力的提升而增加，具有独特数据特性的公司可更有效地实现货币化。在即将到来的无限内容时代，构建信任链和内容质量的策展机制将变得至关重要。
> 
>  **要点总结**:
>  1. 信息和计算的相对价值变化影响软件和数据业务模式。
>  2. 数据和计算量的绝对增加对信任、身份和内容质量产生影响。
>  3. 数据推动了过去十年的成功商业模式，软件和数据互为依存。
>  4. GPT等模型显著提升软件开发效率，推动数据的价值提升。
>  5. 在内容无限的时代，构建信任链和策展质量机制将变得重要。
