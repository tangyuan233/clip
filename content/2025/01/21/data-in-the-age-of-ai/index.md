...
> **摘要**:  
>  本文探讨了人工智能（AI）的爆发对数据及数据业务的影响，提出了两个主要观点：一是信息和计算的相对价值发生了重大变化，影响了软件和数据业务模式；二是数据和计算能力的绝对数量显著增加，对信任、身份、质量和数据策展产生了深远影响。文章回顾了过去十年数据爆炸如何推动最佳商业模式的发展，并指出AI时代数据的价值将进一步提升，尤其是独特数据资产和高质量数据的重要性。此外，生成式AI不仅消耗数据，还生成数据，推动了计算爆炸，进一步加速了数据生态系统的重构。最后，文章讨论了数据信任链的建立、计算能力的扩展以及AI代理在未来的广泛应用。
> 
>  **要点总结**:
>  1. **数据与计算的相对价值变化**：AI的兴起改变了信息和计算的相对价值，影响了软件和数据业务模式。
>  2. **数据爆炸与计算爆炸**：过去十年的数据爆炸推动了商业模式的创新，而生成式AI的出现则进一步加速了计算能力的扩展。
>  3. **数据的价值提升**：在AI时代，独特数据资产和高质量数据（如BloombergGPT和Reddit的内容）变得尤为珍贵，数据质量的重要性超过数据规模。
>  4. **数据信任链的建立**：随着数据量的激增，信任链（签名、来源、身份、质量、策展）成为确保数据可信度的关键。
>  5. **AI代理的广泛应用**：AI代理将在未来广泛应用于各个领域，帮助生成和消费数据，提升生产力，并可能改变权力和财富的分配。
...
![](https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F4f5d7f81-7b28-492d-b4c4-ac5951d4de0f_1572x1203.jpeg)
