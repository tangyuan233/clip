...
> **摘要**: 本文探讨了人工智能（AI）突发性的爆炸性发展如何影响数据及数据商业模式，提出了两个主要观点：首先，信息与计算的相对价值发生了显著变化，因而重新定义了软件和数据的商业模式；其次，全球可用的数据和计算能力呈现出更为剧烈的绝对增长，这影响了信任、身份、质量和策展的相关问题。文章还回顾了数据和数据循环在过去十年最佳商业模式中的重要性，并指出了计算革命会让数据变得更有价值，尤其对拥有独特和潜在数据资产的公司而言。此外，随着生成模型的不仅生成数据也消耗数据，数据的质量和筛选策略也会变得更加重要，最终形成一个新出现的围绕数据构建的商业生态系统。
> 
>  **要点总结**:
>  1. 信息与计算的相对价值显著变化，影响软件与数据商业模式。
>  2. 全球的数据和计算能力的绝对增长影响信任、身份、质量和策展。
>  3. 数据爆炸促进了许多成功商业模式的出现，并提升了数据的价值。
>  4. 生成模型推动了数据的生产与消耗，从而改变数据的使用方式。
>  5. 新的商业生态系统将围绕数据资产的货币化与优化展开。
