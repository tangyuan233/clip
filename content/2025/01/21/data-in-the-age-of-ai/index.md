...
> **摘要**:
>  这篇文章探讨了人工智能（AI）突发爆发如何影响数据及数据业务，作者提出了两个主要观点：一是信息和计算相对价值的重大变化，二是全球数据和计算绝对数量的显著增加。文章回顾了过去十年数据与模型的演变，强调数据循环推动了业务模式的成功。例如，在广告和物流领域，通过廉价存储促进了用户、内容和广告商之间的良性循环。而AI的推动使得软件开发效率惊人提升，尤其是GPT等模型带来了计算能力的爆炸式增长，使数据的价值大幅提升。基于这些变化，建议构建新工具和生态系统，特别是在信任链及数据质量方面，同时也应关注人工智能对人类角色的影响。
> 
>  **要点总结**:
>  1. AI引发信息与计算相对价值及绝对数量的重大变化，对数据业务模式产生深远影响。
>  2. 数据与软件是相辅相成的，廉价数据促进了软件产业发展。
>  3. 数据量的爆炸性增长使得数据的价值显著提升，企业需重新评估其数据资产。
>  4. 数据质量的重要性日益凸显，高质量数据对于商业应用更具价值。
>  5. 人工智能可能对人类角色造成影响，需探索人类在数据驱动世界中的未来。
