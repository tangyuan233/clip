...
> **摘要**: 文章探讨了人工智能爆发对数据及其业务的影响，提出两个主要观点：一是信息与计算的相对价值发生了重大变化，影响软件及数据业务模式；二是全球数据和计算的绝对数量急剧增加，影响信任、身份、质量及内容策展。文章回顾了过去三十年数据存储和利用的演变，分析了数据爆炸如何推动多个成功商业模式的发展，强调数据和软件是密不可分的关系。在探讨生成式AI的崛起及其对数据价值的提升后，作者认为未来将出现计算爆炸，促使数据和计算之间形成新的飞轮效应，并指出在多样化的数据生态下，价值的重塑和信任链的建立是不可或缺的。
> 
>  **要点总结**:
>  1. AI爆发导致信息与计算的相对价值转变，并影响商业模式。
>  2. 数据和计算的绝对数量急剧增加，改变数据价值与获取。
>  3. 数据和软件是互为补充的，二者共同推动商业模式的成功。
>  4. 生成式AI提升了数据的价值，为公司货币化独特数据资产提供机会。
>  5.未来需要构建新的商业生态系统，确保数据质量与信任链的形成。
