...
> **摘要**:
>  本文探讨了人工智能（AI）的快速发展对数据及数据业务的影响，提出了两个主要观点：一是信息和计算的相对价值发生了重大变化，影响了软件和数据业务模式；二是全球数据和计算能力的绝对数量显著增加，对信任、身份、质量和数据管理产生了深远影响。文章回顾了过去十年数据爆炸的历程，指出数据循环是推动最佳商业模式和AI革命的关键因素。随着GPT等生成式AI的出现，数据的价值大幅提升，计算能力也迎来了爆炸式增长。作者还讨论了数据质量、数据资产货币化、信任链的建立以及AI代理的普及对未来数据业务的影响。
> 
>  **要点总结**:
>  1. **数据与计算的相对价值变化**：信息和计算的相对价值发生了重大变化，影响了软件和数据业务模式，数据的价值随着计算能力的提升而增加。
>  2. **数据爆炸与计算爆炸**：全球数据和计算能力的绝对数量显著增加，数据爆炸推动了计算爆炸，生成式AI进一步加速了这一趋势。
>  3. **数据资产的价值提升**：独特数据资产和潜在数据资产的价值在AI时代大幅提升，数据质量的重要性超过数据规模。
>  4. **信任链的建立**：随着数据量的激增，信任链（签名、来源、身份、质量、筛选）成为确保数据可信度的关键。
>  5. **AI代理的普及**：AI代理将广泛应用于各个领域，帮助生成和消费数据，提升生产力，但也可能加剧权力和财富的不平等。
