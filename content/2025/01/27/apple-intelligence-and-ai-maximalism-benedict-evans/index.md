...
> **摘要**:
>  本文探讨了苹果在生成AI领域的策略，认为生成AI应被视为一种商品基础设施，而非单一的平台或产品。文章指出，尽管苹果展示了一系列酷炫的生成AI功能，目前尚未广泛应用，其产品依赖于具备在设备上处理能力的iPhone 15 Pro。苹果的战略与普遍的"AI最大主义"观点相对立，后者认为多功能聊天机器人将取代众多复杂软件，但苹果认为其生成AI技术将嵌入更广泛的应用场景中，并通过将其分解为独立功能来运作。苹果不打算提供开放的聊天机器人输入，而是在隐私保护下利用设备获取的用户背景信息来增强应用。此外，文章讨论了苹果如何通过建立自己的基础模型来规避品牌风险，并将第三方模型（如OpenAI的ChatGPT）作为互换插件，以降低生成AI的应用门槛和成本，从而推动生成AI在边缘设备上的落地。
> 
>  **要点总结**:
>  1. 苹果将生成AI视为商品基础设施，而非单一产品，强调其更广泛的应用场景。
>  2. 该技术依赖于用户设备背景信息的嵌入，并通过特定功能实现。
>  3. 苹果不提供开放式聊天机器人接口，而是将用户请求和隐私保护分开处理。
>  4. 苹果以自己的基础模型降低与第三方模型的品牌风险，并将其视为可互换的插件。
>  5. 文章讨论了生成AI的未来将取决于其作为功能集成到产品中的方式。
