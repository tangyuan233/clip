...
> **摘要**: 本文探讨了苹果在生成性人工智能（AI）方面的战略，强调其将大型语言模型（LLM）视为商品基础设施而非平台或产品的观点。虽然苹果的AI功能尚未广泛可用，但它已经提出了一种不同于当前AI炒作的理论，主张生成性AI应该嵌入于提供用户上下文的系统中，并解耦为易于在高效小设备上运行的独立特性。苹果设计了一种仅用于特定功能的基础模型，通过用户设备收集的上下文信息进行操作，而不是作为一个问答的神谕。苹果的模式注重隐私并避免了生成性AI的开放式问询带来的潜在品牌风险。尽管目前市场各玩家几乎都具备相似的基础设施，如何利用这些生成性AI技术仍然是未来的关键问题。本文指出，苹果可能会在未来逐渐将更多用例从第三方转移到自家的模型中，这为整个行业的竞争格局带来了新的思考。
> 
>  **要点总结**:
>  1. 苹果认为生成性AI是商品技术，需嵌入系统并解耦成独立特性。
>  2. 苹果的基础模型在设计上不提供开放式问询，而是专注于特定功能，增强用户隐私。
>  3. 苹果与OpenAI的合作表明，AI基础设施面临品牌风险，苹果选择安全的方式进行操作。
>  4. 市场上生成性AI的企业将依赖其上下文信息的不同处理策略来构建竞争优势。
>  5. 未来生成性AI可能不会是单一公司主导的工具，而是构成众多产品内部功能的通用技术。
