...
> **摘要**:
>  本文探讨了苹果在生成AI领域的定位，强调其与现有市场的不同之处。苹果认为大型语言模型（LLMs）是一种商品基础设施，而不是独立的平台或产品。尽管苹果的智能功能尚未广泛推广，且需要新型号设备支持，但其提出的战略和观点注重将生成AI融入系统中，以便更好地理解用户并提供个性化特性。苹果通过构建上下文模型并将生成AI分解为更小的特性，来提高其操作效率，并确保用户的隐私安全。同时，苹果对开放AI的整体依赖并非绝对，而是通过不同模型的分工与综合来实现不同需求。尽管生成AI市场尚在演变中，苹果坚持认为这种技术将在多家公司中推动功能的整合与发展，从而重塑产品形态。
> 
>  **要点总结**:
>  1. 苹果把生成AI视为商品基础设施，强调其技术性而非产品性。
>  2. 其生成AI仅在iPhone 15 Pro等新设备中运作，且功能嵌入用户系统以提升效率。
>  3. 苹果注重隐私，将上下文模型与开放式模型进行区分，确保数据安全。
>  4. 依赖于其他模型的同时，苹果通过不同的问题导向不同的平台，保持灵活性。
>  5. 生成AI的未来发展在于集成与特性的简化，而不是取代现有软件。
