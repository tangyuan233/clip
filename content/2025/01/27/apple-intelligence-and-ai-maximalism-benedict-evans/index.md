...
> **摘要**:
>  文章探讨了苹果在生成人工智能（AI）方面的战略，认为其与当前流行的AI最大化观点截然不同。首先，苹果尚未正式推出智能功能，现有的技术仍局限于新一代iPhone。苹果强调生成AI是一种基础技术，而非单一的产品或平台，认为它应嵌入不同用户环境中并分解为多个小功能。文章中指出，苹果建立了自己的基础模型，并通过特定的界面与用户交互，而不是简单引导用户获取开放式响应。这种“上下文模型”和“世界模型”的分离，确保了用户隐私，且苹果试图与开放AI相互配合，而不是直接竞争。尽管生成AI被视为商品，苹果的长远发展将依赖于如何利用这些技术与用户设备的集成，文章最后提出了未来生成AI的发展仍充满不确定性，各大公司仍在探索其应用潜力。
> 
>  **要点总结**:
>  1. 苹果认为生成AI应作为基础技术嵌入系统中，而非单独的产品或平台。
>  2. 生成AI的应用范围被分为“上下文模型”和“世界模型”，以保护用户隐私。
>  3. 苹果建立了自己的基础模型，与用户交互的功能设计上注重隐私和上下文相关性。
>  4. 生成AI被视为商品，未来发展取决于如何在用户设备上更好地集成和利用这些技术。
>  5. 未来生成AI的市场应用仍存在不确定性，各公司正探索不同方向。
