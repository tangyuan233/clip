...
> **摘要**:  
>  苹果在生成式AI领域提出了一种不同于主流“AI最大化”观点的策略，认为大语言模型（LLM）是基础设施而非独立产品或平台。苹果通过将AI技术嵌入系统，赋予其用户上下文信息，并将其解构为多个小型功能，运行在用户设备上，而非依赖云端计算。苹果的AI模型不提供开放式聊天机器人功能，而是通过特定功能（如邮件优先级排序、摘要生成）来体现AI的价值。此外，苹果将AI分为“上下文模型”和“世界模型”，前者基于用户设备上的私有数据，后者则依赖第三方模型（如ChatGPT）。苹果的策略表明，生成式AI的未来可能更多是作为技术嵌入现有产品，而非独立工具。
> 
>  **要点总结**:
>  1. **AI作为基础设施**：苹果认为生成式AI是基础设施，而非独立产品或平台，强调其嵌入系统和用户上下文的重要性。
>  2. **设备端AI处理**：苹果的AI功能主要在用户设备上运行，减少对云端计算的依赖，降低边际成本。
>  3. **上下文与世界模型分离**：苹果将AI分为“上下文模型”（基于用户设备数据）和“世界模型”（依赖第三方模型），确保用户隐私与功能分离。
>  4. **AI功能的解构与集成**：苹果将AI技术解构为多个小型功能，集成到现有产品中，而非提供开放式聊天机器人。
>  5. **AI未来的不确定性**：生成式AI仍处于早期阶段，产品市场契合度尚未明确，未来可能因技术突破或市场需求变化而发生重大调整。
