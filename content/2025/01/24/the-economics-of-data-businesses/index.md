...
> **摘要**:
>  本文探讨了数据企业作为一种特殊的商业模式，强调数据是企业的核心产品，成功的数据企业围绕独特的数据资产构建。作者提出数据企业模型的三个基本真理：其一，数据是价值创造的核心；其二，掌控数据才能获取经济利益；其三，数据企业初期发展缓慢，然而一旦建立起数据资产和销售渠道，整体速度会加快并持续增长。为了获得和提升数据，企业采用多个策略，包括原始数据收集、数据聚合和许可等。数据企业的市场地位坚固，难以被替代，因为它们在获取和运用数据方面形成了强大的网络效应和其他竞争优势。然而，成功的数据企业相对稀缺，创建新的数据资产需要较高的门槛，并常常需要创新的类别创建。
> 
>  **要点总结**:
>  1. 数据企业是以数据为核心产品的企业，成功建立在独特的数据资产之上。
>  2. 控制数据是获取经济利益的关键，依赖单一数据来源风险高。
>  3. 数据企业的发展通常起步缓慢，但一旦建立起基础设施，便会加速增长。
>  4. 成功的数据企业的市场地位稳固，难以被其他企业替代。
>  5. 创立成功的数据企业并非易事，新的数据资产创建需高度创新和匹配行业需求。
