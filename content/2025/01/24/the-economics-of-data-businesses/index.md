...
> **摘要**:
>  本文探讨了数据业务这一商业模式，强调数据是其核心产品，成功的数据公司均建立在独特的数据资产之上。文章介绍了多种获取数据的方法，包括直接收集、聚合、转化和核心业务产生等。关键观点包括：掌握数据是价值的控制权，数据业务起步慢但会加速增长，以及数据业务难以被取代且非常稀缺。作者还指出，虽然技术投资重视增长率，数据业务因前期投资大而难以融资，但一旦建立起数据资产，将会有显著的长期收益。成熟的数据业务通常形成双头垄断，表现出超强的行业防御力。
> 
>  **要点总结**:
>  1. 数据企业的核心在于数据，成功企业围绕独特的数据资产运作。
>  2. 控制数据的公司能够捕捉价值，而依赖他人数据的公司风险较高。
>  3. 数据业务起步较慢，但随着数据积累，会加速成长。
>  4. 数据业务具备强大的防御能力，难以被新竞争者取代。
>  5. 成功的数据业务稀缺且建立门槛高，创造新数据资产需要创新。
