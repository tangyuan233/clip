...
> **摘要**:
>  本文探讨了数据商业模式的关键特征，尽管许多大型科技公司将数据作为核心产品，但相关研究仍较少。首先，作者界定数据业务为以数据为核心产品的公司，强调数据是价值创造的基本单位。其次，成功的数据企业需围绕独特的数据资产构建，获取数据的方法包括强力收集、聚合、授权和交互等多种方式。接下来，强调控制数据的企业能更好地捕获价值，而单一数据来源则增加了失败风险。此外，数据企业通常起步缓慢，但随着数据资产的增长，会加速其增长和市场占有率。最后，作者指出成熟的数据企业具有显著的防御能力，难以被替代，并且稀缺的数据资产创造成功数据企业的机会有限，这使得数据商业模式的投资潜力受限。
> 
>  **要点总结**:
>  1. 数据业务是以数据为核心产品的公司，其价值创造单位为数据。
>  2. 成功的数据企业依赖独特的数据资产，通过多样化方法收集数据。
>  3. 控制数据是捕获价值的关键，依赖单一来源会增加风险。
>  4. 数据企业发展初期缓慢，但随着资产和市场扩展将加速增长。
>  5. 成熟的数据企业因其结构优势和稀缺性而难以被替代。
